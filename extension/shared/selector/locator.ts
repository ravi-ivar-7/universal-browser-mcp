...
export class SelectorLocator {
  ...
  async locate(
    ...
    // Extract fingerprint validation configuration
    ...
    if (typeof target.selector === 'string' && target.selector.trim()) {
      ...
      if (ensured) {
        ...
        // Fingerprint validation: Skip if mismatch, continue to try other candidates
        const fingerprintOk =
          !fingerprintToVerify ||
        ...
        // Fingerprint mismatch, continue attempting other candidates
      }
    }
...
/**
 * Create Chrome Extension Transport Layer
 */
...
/**
 * Create Chrome Extension Selector Locator
 */
