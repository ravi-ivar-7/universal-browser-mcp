...
(function () {
      ...
      root.innerHTML = `
        <div id="__rr_rec_panel" style="background: rgba(220,38,38,0.95); color: #fff; padding:8px 10px; border-radius:8px; display:flex; align-items:center; gap:8px; box-shadow:0 4px 16px rgba(0,0,0,0.2);">
          ...
          <label style="display:inline-flex; align-items:center; gap:4px; font-size:12px;">
            <input id="__rr_hide_values" type="checkbox" style="vertical-align:middle;" />Mask Input
          </label>
          <label style="display:inline-flex; align-items:center; gap:4px; font-size:12px;">
  ...
  class ContentRecorder {
    ...
    async _flush() {
      ...
      // Take a snapshot of the batch and clear it immediately to avoid race conditions.
      // If send fails, we'll put them back.
      const snapshot = this.batch.splice(0, this.batch.length);
      const steps = snapshot.map((s) => {
        // sanitize internal fields before sending to background
        const { _recordingRef, ...rest } = s || {};
        return rest;
      });

      const sessionId = this.sessionBuffer && this.sessionBuffer.id ? this.sessionBuffer.id : null;
      ...
      if (!ok) {
        // Restore steps to the beginning of the batch if send failed
        this.batch.unshift(...snapshot);
      }
      return ok;
    }
