/**
 * @fileoverview RR Port-RPC Client Hook (React)
...
import type { JsonObject, JsonValue } from '@/entrypoints/background/record-replay/domain/json';
import type { RunEvent } from '@/entrypoints/background/record-replay/domain/events';
import type { RunId } from '@/entrypoints/background/record-replay/domain/ids';
import {
    RR_PORT_NAME,
    createRpcRequest,
    isRpcEvent,
    isRpcResponse,
    type RpcMethod,
} from '@/entrypoints/background/record-replay/engine/transport/rpc';
...
/** Hook configuration */
export interface UseRRRpcOptions {
...
/**
 * RR Port-RPC client (React Hook)
 */
export function useRRRpc(options: UseRRRpcOptions = {}): UseRRRpc {
    // Configuration
    ...
    const requestRef = useRef<UseRRRpc['request']>(async () => {
    ...
    const rehydrateSubscriptions = useCallback(async (): Promise<void> => {
        ...
        for (const runId of desiredSubscriptionsRef.current) {
            try {
                ...
                await requestRef.current('rr.subscribe', params).catch(() => {
    ...
    const scheduleReconnect = useCallback((): void => {
        ...
        if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
            ...
            setError('RR RPC: max reconnect attempts reached');
            return;
        }
    ...
    const handlePortDisconnect = useCallback((): void => {
        ...
        const reason = disconnectReason
            ? `RR RPC disconnected: ${disconnectReason}`
            : 'RR RPC disconnected';
            ...
            if (isRpcEvent(msg)) {
                ...
                for (const listener of eventListenersRef.current) {
                    try {
                        ...
                        console.error('[useRRRpc] Event listener error:', e);
                    }
                }
            }
        },
    ...
    const connectInternal = useCallback(async (): Promise<chrome.runtime.Port | null> => {
        ...
        const promise = (async () => {
            ...
            try {
                ...
                const p = chrome.runtime.connect({ name: RR_PORT_NAME });
                portRef.current = p;
    ...
    const disconnectInternal = useCallback(
        (reason?: string): void => {
            ...
            rejectAllPending(reason || 'RR RPC: client disconnected');
    ...
    const ensureConnectedInternal = useCallback(async (): Promise<chrome.runtime.Port> => {
        ...
        if (!p) {
            throw new Error(`RR RPC: Failed to connect. ${lastError || 'Unknown error'}`);
        }
        return p;
    ...
    const request = useCallback<UseRRRpc['request']>(
    ...
    const subscribe = useCallback(
        async (runId: RunId | null = null): Promise<boolean> => {
            ...
            try {
                const params: JsonObject = runId === null ? {} : { runId };
                await request('rr.subscribe', params);
                return true;
            } catch (error) {
    ...
    const unsubscribe = useCallback(
        async (runId: RunId | null = null): Promise<boolean> => {
            ...
            try {
                const params: JsonObject = runId === null ? {} : { runId };
                await request('rr.unsubscribe', params);
                return true;
            } catch (error) {
                setError(toErrorMessage(error));
                return false;
