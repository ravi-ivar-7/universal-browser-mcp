  ...
  if (world === ExecutionWorld.MAIN) {
    ...
    const wrapped = `(() => {
      try {
        ...
        window.addEventListener('universal-browser-mcp:execute', (ev) => {
          ...
          try {
            ...
            window.dispatchEvent(new CustomEvent('universal-browser-mcp:response', { detail: { requestId, data: result } }));
          } catch (err) {
            window.dispatchEvent(new CustomEvent('universal-browser-mcp:response', { detail: { requestId, error: String(err && (err as any).message || err) } }));
          }
        });
    ...
    if (active && active.id) {
      try {
        if (rec.sourceType === 'CSS') {
          ...
          chrome.tabs.sendMessage(active.id, { type: 'universal-browser-mcp:cleanup' }).catch(() => {});
        }
