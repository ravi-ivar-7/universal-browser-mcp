/**
 * @fileoverview Record-Replay composition root (bootstrap)
 * @description
...
import type { RunRecord } from './domain/events';
...
/**
 * RecordReplay Runtime Handle
 */
export interface RecordReplayRuntime {
  ownerId: string;
  storage: StoragePort;
  events: EventsBus;
...
let runtime: RecordReplayRuntime | null = null;
let bootstrapPromise: Promise<RecordReplayRuntime> | null = null;
...
async function createEphemeralTab(logger: Logger): Promise<number> {
  ...
  logger.debug(`[RR] Allocated ephemeral tab ${tab.id}`);
  return tab.id;
}
...
async function safeRemoveTab(tabId: number, logger: Logger): Promise<void> {
  try {
    ...
    logger.debug(`[RR] Failed to close tab ${tabId}:`, e);
  }
}
...
async function failRun(
  ...
  try {
    ...
    deps.logger.error(`[RR] Failed to patch run "${runId}" as failed:`, e);
    return;
  }
  ...
  try {
    ...
    deps.logger.warn(`[RR] Failed to append run.failed for "${runId}":`, e);
  }
}
...
function createDefaultRunExecutor(deps: {
  ...
  return async (item: RunQueueItem): Promise<void> => {
    ...
    if (!run) {
      const msg = `[RR] RunRecord not found for queue item "${runId}"`;
    ...
    try {
      ...
      deps.logger.debug(`[RR] Failed to patch run "${runId}" attempt/tabId:`, e);
    }
...
/**
 * Bootstrap RR Runtime
 * @returns Runtime handle
 */
export async function bootstrapRecordReplay(): Promise<RecordReplayRuntime> {
  ...
  bootstrapPromise = (async () => {
    ...
    logger.debug(`[RR] Owner ID: ${ownerId}`);
    ...
    const keepalive = {
      acquire: (tag: string) => acquireKeepalive(`rr:${tag}`),
    };

    // 7) PluginRegistry - register Action handlers as Nodes
    const plugins = new PluginRegistry();
    const registeredNodes = registerActionNodes(plugins, {
      // Exclude control directives that runner doesn't support
      exclude: [...DEFAULT_EXCLUDE_LIST],
    });
    logger.debug(`[RR] Registered ${registeredNodes.length} action handlers as nodes`);
    ...
    try {
      ...
      logger.info('[RR] Running crash recovery...');
      ...
      logger.info('[RR] Bootstrap complete');
    } catch (e) {
      await cleanup();
      throw e;
    }
 ...
 */
export function getRecordReplayRuntime(): RecordReplayRuntime | null {
  return runtime;
}

/**
 * Check if RR is running
 */
export function isRecordReplayRunning(): boolean {
  return runtime !== null;
}
