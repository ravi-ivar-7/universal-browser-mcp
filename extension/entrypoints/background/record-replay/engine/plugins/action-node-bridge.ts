/**
 * @fileoverview ActionHandler -> NodeDefinition adapter
 * @description Bridges legacy Action handlers into the RR PluginRegistry.
 *
 * Design notes:
 * - Runtime requires variable mutations to be represented as varsPatch so they are auditable in the event log.
 * - Handlers mutate ctx.vars directly, so we run them against a cloned VariableStore and diff it.
 * - Cross-node state (tabId/frameId changes from switchFrame/openTab/switchTab) is persisted in internal vars.
 *
 * WARNING: This adapter accesses handler internals and may need updates if types change.
 */
...
// Internal run-scoped state keys used to emulate "mutable context" across nodes.
const DEFAULT_TAB_ID_VAR = '__rr_tabId';
const DEFAULT_FRAME_ID_VAR = '__rr_frameId';

export interface ActionNodeBridgeOptions {
  /**
   * Whether to emit v2 ActionExecutionResult.output into NodeExecutionResult.outputs.
   * Defaults to true.
   */
   ...
   * Defaults are "__rr_tabId" and "__rr_frameId".
   */
  stateVars?: {
    tabIdVar?: string;
    frameIdVar?: string;
  ...
  /**
   * Execution flags forwarded into ActionExecutionContext.execution.
   * Keep default undefined to preserve handler behavior.
   */
...
function mapActionErrorCode(code: ActionErrorCode): RRErrorCode {
  switch (code) {
    ...
    // Runtime doesn't currently have dedicated codes for these.
...
function toRRErrorFromActionError(error: ActionError): RRError {
...
function toActionPolicy(policy: NodePolicy | undefined): ActionPolicy | undefined {
  ...
  return {
    ...
    ...(policy.onError
      ? (() => {
        // Only supports goto by edge label. Node-target goto can't be represented.
        ...
        if (policy.onError.kind === 'goto') {
          ...
          // Node target can't be represented, fall through to stop
          return { onError: { kind: 'stop' } as ActionPolicy['onError'] };
        }
        if (policy.onError.kind === 'retry') {
          // Retry policy on action.policy.retry; keep onError as stop to avoid double semantics.
          return { onError: { kind: 'stop' } as ActionPolicy['onError'] };
        }
...
export function adaptActionHandlerToNodeDefinition<T extends ExecutableActionType>(
  ...
  return {
    ...
    execute: async (ctx: NodeExecutionContext, node): Promise<NodeExecutionResult> => {
      ...
      const handlerVars = deepClone(beforeVars) as unknown as Record<string, unknown>;

      const handlerCtx: ActionExecutionContext = {
        vars: handlerVars as unknown as ActionExecutionContext['vars'],
        ...
        pushLog: (entry) => {
          try {
            ctx.log('debug', 'handler.pushLog', safeJsonValue(entry));
          } catch {
            // ignore
          }
        },
      ...
      try {
        result = await handler.run(handlerCtx, action);
      } catch (e) {
        return {
          status: 'failed',
          error: createRRError(
            RR_ERROR_CODES.INTERNAL,
            `Handler "${handler.type}" threw: ${toErrorMessage(e)}`,
          ),
        };
      }
      ...
      if (result.status === 'failed') {
        const err = result.error
          ? toRRErrorFromActionError(result.error)
          : createRRError(RR_ERROR_CODES.INTERNAL, `Handler "${handler.type}" failed`);
      ...
      if (result.status === 'paused') {
        return {
          ...
          error: createRRError(
            ...
            `Handler "${handler.type}" returned paused (not supported in NodeExecutionResult)`,
          ),
        };
      }

      // Runner does not support scheduler control directives (foreach/while).
      if (result.control) {
        return {
          status: 'failed',
          error: createRRError(
            RR_ERROR_CODES.UNSUPPORTED_NODE,
            `Control directive "${result.control.kind}" is not supported by the runner`,
      ...
      if (typeof handlerCtx.frameId === 'number' && Number.isFinite(handlerCtx.frameId)) {
        handlerVars[frameIdVar] = handlerCtx.frameId;
      } else {
        delete handlerVars[frameIdVar];
      }

      if (typeof result.newTabId === 'number' && Number.isFinite(result.newTabId)) {
        handlerVars[tabIdVar] = result.newTabId;
      }

      const afterVars = toJsonRecord(handlerVars);
