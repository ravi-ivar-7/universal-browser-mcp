...
import {
  RR_KEEPALIVE_PORT_NAME,
  type KeepaliveMessage,
} from '@/common/rr-keepalive-protocol';

// ==================== Runtime Control Protocol ====================

const KEEPALIVE_CONTROL_MESSAGE_TYPE = 'rr_keepalive.control' as const;
...
class OffscreenKeepaliveControllerImpl implements KeepaliveController {
  ...
  private handleConnect = (port: chrome.runtime.Port): void => {
    if (port.name !== RR_KEEPALIVE_PORT_NAME) return;
