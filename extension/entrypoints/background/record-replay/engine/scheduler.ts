...
class ExecutionOrchestrator {
  private static activeRuns = new Map<string, ExecutionOrchestrator>();

  public static stopRun(runId: string) {
    const run = this.activeRuns.get(runId);
    if (run) run.stop();
  }

  public static pauseRun(runId: string) {
    const run = this.activeRuns.get(runId);
    if (run) run.paused = true;
  }

  public static resumeRun(runId: string) {
    const run = this.activeRuns.get(runId);
    if (run) run.paused = false;
  }

  public static getActiveRuns() {
  ...
  constructor(
    ...
    this.logger = new RunLogger(this.runId, flow.id);
  ...
  public stop() {
    ...
    this.logger.push({
      stepId: 'run_stopped_manually',
      status: 'failed',
      message: 'Execution stopped by user',
    });
    ExecutionOrchestrator.activeRuns.delete(this.runId);
  }

  async run(): Promise<RunResult> {
    try {
      ExecutionOrchestrator.activeRuns.set(this.runId, this);
  ...
  private async traverseDag(): Promise<RunResult> {
    ...
    const ctx: ExecCtx = {
      vars: this.vars,
      tabId: this.tabId!,
      logger: (entry: RunLogEntry) => this.logger.push(entry),
    ...
    while (currentId) {
      ...
      // Pause handling with high responsiveness
      while (this.paused && !this.terminated) {
  ...
  private async cleanup() {
    ...
    this.logger.push({
      stepId: 'log_run_end',
      status: this.failed === 0 && !this.terminated ? 'success' : 'failed',
      message: this.terminated ? 'Replay stopped by user' : this.failed === 0 ? 'Replay completed successfully' : 'Replay failed',
    });
    try {
      try {
...
export function runFlow(flow: Flow, options: RunOptions = {}): { runId: string; promise: Promise<RunResult> } {
...
export function stopRun(runId: string) {
  ExecutionOrchestrator.stopRun(runId);
}

export function pauseRun(runId: string) {
  ExecutionOrchestrator.pauseRun(runId);
}

export function resumeRun(runId: string) {
  ExecutionOrchestrator.resumeRun(runId);
}

export function getActiveRuns() {
  return ExecutionOrchestrator.getActiveRuns();
}
