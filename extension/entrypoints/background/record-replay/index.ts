      ...
      switch (message?.type) {
        ...
        case BACKGROUND_MESSAGE_TYPES.RR_RUN_FLOW: {
          getFlow(message.flowId)
            .then((flow) => {
              if (!flow) return sendResponse({ success: false, error: 'flow not found' });
              const { runId, promise } = runFlow(flow, message.options || {});
              sendResponse({ success: true, result: { runId } });
              // Continue execution in background
              promise.catch(e => console.error('Run failed:', e));
            })
            .catch((e) => sendResponse({ success: false, error: e?.message || String(e) }));
          return true;
        }
        case BACKGROUND_MESSAGE_TYPES.RR_STOP_RUN: {
          stopRun(message.runId);
          sendResponse({ success: true });
          return true;
        }
        case BACKGROUND_MESSAGE_TYPES.RR_PAUSE_RUN: {
          pauseRun(message.runId);
          sendResponse({ success: true });
          return true;
        }
        case BACKGROUND_MESSAGE_TYPES.RR_RESUME_RUN: {
          resumeRun(message.runId);
          sendResponse({ success: true });
          return true;
        }
        case BACKGROUND_MESSAGE_TYPES.RR_GET_ACTIVE_RUNS: {
          sendResponse({ success: true, runs: getActiveRuns() });
          return true;
        }
