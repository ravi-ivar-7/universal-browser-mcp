/**
 * @fileoverview RunRecord Persistence
...
export function createRunsStore(): RunsStore {
  return {
    async list(): Promise<RunRecord[]> {
      return withTransaction(RR_STORES.RUNS, 'readonly', async (stores) => {
        const store = stores[RR_STORES.RUNS];
    ...
    async get(id: RunId): Promise<RunRecord | null> {
      return withTransaction(RR_STORES.RUNS, 'readonly', async (stores) => {
        const store = stores[RR_STORES.RUNS];
        return new Promise<RunRecord | null>((resolve, reject) => {
          const request = store.get(id);
          request.onsuccess = () => resolve((request.result as RunRecord) ?? null);
          request.onerror = () => reject(request.error);
        });
      });
    },
    ...
    async save(record: RunRecord): Promise<void> {
      ...
      return withTransaction(RR_STORES.RUNS, 'readwrite', async (stores) => {
        const store = stores[RR_STORES.RUNS];
    ...
    async patch(id: RunId, patch: Partial<RunRecord>): Promise<void> {
      return withTransaction(RR_STORES.RUNS, 'readwrite', async (stores) => {
        const store = stores[RR_STORES.RUNS];

        // Read existing record first
        const existing = await new Promise<RunRecord | null>((resolve, reject) => {
          const request = store.get(id);
          request.onsuccess = () => resolve((request.result as RunRecord) ?? null);
          request.onerror = () => reject(request.error);
        });
