...
import type { RunEvent, RunEventInput, RunRecord } from '../domain/events';
  ...
  return {
    ...
    async append(input: RunEventInput): Promise<RunEvent> {
      return withTransaction(
        [RR_STORES.RUNS, RR_STORES.EVENTS],
        'readwrite',
        async (stores) => {
          const runStore = stores[RR_STORES.RUNS];
          const eventStore = stores[RR_STORES.EVENTS];

          // Step 1: Read nextSeq from RunRecord (single source of truth)
          const run = await idbRequest<RunRecord | undefined>(
            runStore.get(input.runId),
          ...
          await idbRequest(eventStore.add(event), `append.addEvent(${input.runId}, seq=${seq})`);
    ...
    async list(runId: RunId, opts?: { fromSeq?: number; limit?: number }): Promise<RunEvent[]> {
      return withTransaction(RR_STORES.EVENTS, 'readonly', async (stores) => {
        const store = stores[RR_STORES.EVENTS];
