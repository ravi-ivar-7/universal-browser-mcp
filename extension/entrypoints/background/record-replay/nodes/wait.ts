...
export const waitNode: NodeRuntime<StepWait> = {
  ...
  run: async (ctx: ExecCtx, step: StepWait) => {
    ...
    if ('text' in cond) {
      ...
      await waitForNetworkIdle(total, idle, ctx.isTerminated);
    } else if ('navigation' in cond) {
      await waitForNavigation((s as any).timeoutMs, undefined, ctx.isTerminated);
    } else if ('sleep' in cond) {
      const targetMs = Math.max(0, Number(cond.sleep ?? 0));
      let elapsedMs = 0;
