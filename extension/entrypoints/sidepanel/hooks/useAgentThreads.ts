...
function isAttachmentMetadata(value: unknown): value is AttachmentMetadata {
    ...
    // Allow optimistic local attachments (they have previewUrl)
    if (typeof v.previewUrl === 'string') return true;
...
export function useAgentThreads(options: UseAgentThreadsOptions) {
    ...
    }, [options.messages, options.requestState, options.currentRequestId]);
